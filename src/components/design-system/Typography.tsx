    ...
    'font-bold',
    {
        variants: {
            ...
            color: {
                white: 'text-white',
                dark: 'text-slate-900',
                gradient: 'bg-gradient-to-r from-blue-500 to-cyan-400 bg-clip-text text-transparent',
...
export function Heading({as = 'h2', level, color, align, className, children, ...props}: HeadingProps) {
    const Component = as
    return (
        <Component className={cn(headingVariants({level, color, align}), className)} {...props}>
            {children}
        </Component>
    )
}
    ...
    '',
    {
        variants: {
            ...
            color: {
                ...
                secondary: 'text-slate-200', // Improved contrast from 300
                muted: 'text-slate-300', // Improved contrast from 400
...
export function Text({as = 'p', size, color, weight, className, children, ...props}: TextProps) {
    const Component = as
    return (
        <Component className={cn(textVariants({size, color, weight}), className)} {...props}>
            {children}
        </Component>
    )
}
