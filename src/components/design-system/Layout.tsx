...
export function Section({ spacing = 'lg', variant = 'default', className, children, ...props }: SectionProps) {
    const spacings = {
        sm: 'py-12',
        md: 'py-16',
        lg: 'py-20',
    ...
    return (
        <section className={cn(spacings[spacing], variants[variant], className)} {...props}>
            {children}
        </section>
    )
}
...
export function Card({ variant = 'default', hover = true, className, children, ...props }: CardProps) {
    const baseStyles = 'rounded-xl p-8 flex flex-col h-full transition-all duration-300 relative overflow-hidden'
